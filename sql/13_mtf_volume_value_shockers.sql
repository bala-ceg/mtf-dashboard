-- =====================================================
-- 13_mtf_volume_value_shockers.sql
-- Identify MTF volume and value shockers (anomalies)
...
mtf_data AS (
    -- Get MTF data with market cap categories
    SELECT
        d.trade_date,
        d.symbol,
        s.market_cap_cr,
...
stock_stats AS (
    SELECT
        ...
        ROUND(MIN(mtf_amount_cr), 2) AS min_mtf_cr,
        ROUND(MAX(mtf_amount_cr), 2) AS max_mtf_cr
    FROM mtf_data
    WHERE mtf_amount_cr IS NOT NULL
...
daily_analysis AS (
    SELECT
        ...
        ROUND(d.mtf_amount_cr, 2) AS mtf_amount_cr,
        ROUND(s.avg_amount_10d, 2) AS avg_amount_10d,
        ROUND(s.stddev_amount_10d, 2) AS stddev_amount_10d,
        ROUND((d.mtf_amount_cr - s.avg_amount_10d) / NULLIF(s.stddev_amount_10d, 0), 2) AS z_score,
        CASE
            WHEN (d.mtf_amount_cr - s.avg_amount_10d) / NULLIF(s.stddev_amount_10d, 0) > 2 THEN 'High Positive Shocker'
            WHEN (d.mtf_amount_cr - s.avg_amount_10d) / NULLIF(s.stddev_amount_10d, 0) < -2 THEN 'High Negative Shocker'
            WHEN (d.mtf_amount_cr - s.avg_amount_10d) / NULLIF(s.stddev_amount_10d, 0) > 1 THEN 'Moderate Positive Shocker'
            WHEN (d.mtf_amount_cr - s.avg_amount_10d) / NULLIF(s.stddev_amount_10d, 0) < -1 THEN 'Moderate Negative Shocker'
            ELSE 'Normal'
        END AS shocker_type,
        ROUND(d.mtf_amount_cr - s.avg_amount_10d, 2) AS deviation_from_avg_cr
    FROM mtf_data d
...
ranked_shockers AS (
    SELECT
        ...
        da.z_score,
        da.shocker_type,
        da.deviation_from_avg_cr,
        ROW_NUMBER() OVER (
            PARTITION BY da.market_cap_category, da.trade_date
            ORDER BY ABS(da.z_score) DESC, ABS(da.deviation_from_avg_cr) DESC
...
SELECT
    ...
    shocker_type,
    ROUND(deviation_from_avg_cr, 2) AS deviation_from_avg_cr
